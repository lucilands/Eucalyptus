...
int main() {
    ...
    Eucalyptus::Panel panel = Eucalyptus::Panel(Eucalyptus::Vector2f(400, 200), Eucalyptus::Vector2f(1080/2 - 200, 720/2 - 100), 0, Eucalyptus::Vector4f(1.0f, 1.0f, 1.0f, 0.5f));
    ...
    while (window.IsRunning()) {
        ...
        main_scene.GetCamera()->position = {cam_pos.x + (camera_speed * (-window.input.held_keys['a'] + window.input.held_keys['d']) * window.GetDeltaTime()),
                                            cam_pos.y + (camera_speed * (-window.input.IsKeyHeld(Eucalyptus::Input::KeyCode::CONTROL) + window.input.IsKeyHeld(Eucalyptus::Input::KeyCode::SPACE)) * window.GetDeltaTime()),
